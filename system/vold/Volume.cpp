...
void Volume::setState(int state) {
    ...
    // namko: Possibly not the best plate to check but it seems to work properly.
    if (oldState == Volume::State_Mounted && 
            (mState == Volume::State_Idle || mState == Volume::State_NoMedia) && 
...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
        ...
        bool isFat = false, isNtfs = false, isExt4 = false, isValid = false;
        ...
        // namko: All mounts (sdcard, extsd, udisk) are RW.
        ...
        if (isFat) {
            ...
            if (Ntfs::doMount(devicePath, "/mnt/secure/staging", false, false,
                    AID_SYSTEM, gid, 0702, true)) {
            ...
            SLOGW("%s uses an unknown filesystem, skipping\n", devicePath);
            continue;
        }
